class Solution {
    public int[] productExceptSelf(int[] nums){
        ...
        int curr=1;
        for(int i=0;i<n;i++){ // This loop builds the product of all elements to the left of i.
            ans[i]=ans[i]*curr;
            curr=curr*nums[i];
        }
        
        curr=1;
        for(int i=n-1;i>=0;i--){ // this build the product of all elements to the right of i.
            ans[i]=ans[i]*curr;
            curr=curr*nums[i];
        }
